...
| 3 | Ventas — grid de venta rápida, carrito, fiado, nota para WhatsApp | ⏳ Próxima etapa |
| 4 | Clientes — historial, saldo fiado, recordatorio de inactivos | ⏳ Pendiente |
...
src/
  db/
    ...
    repositories/
      insumosRepo.js       CRUD de insumos + ajustarStock (compra/merma/ajuste) + historial
  ...
  pages/
    ...
    InicioPage.jsx / VenderPage.jsx / ClientesPage.jsx / GastosPage.jsx / TareasPage.jsx
...
Cuando se construya el módulo de Ventas, cada venta va a recorrer `plato.receta` y llamar a `ajustarStock(insumoId, -cantidad, { tipo: 'venta' })` por cada insumo — la función ya existe en `insumosRepo.js` lista para reutilizarse.

## Próximos pasos sugeridos

1. **Ventas**: grid de botones grandes con platos/combos → carrito → cerrar venta (pago inmediato o fiado) → nota de texto para compartir por WhatsApp (`navigator.share` con fallback a `wa.me`).
2. **Clientes**: se necesita un store `clientes` y otro `ventas` (cabecera + items) para poder calcular historial y saldo.
3. **Gastos**: store `gastos` simple, con filtro por fecha.
4. **Inicio**: una vez existan `ventas` y `gastos`, agregar el resumen del día/semana/mes.
5. **Organización**: stores `tareas` y `publicaciones`, sin integraciones externas.
